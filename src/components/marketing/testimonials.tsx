      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-7 lg:px-10">
        ...
        <div className="mt-12 grid grid-cols-1 gap-px overflow-hidden rounded-2xl bg-ivory-300 sm:mt-14 lg:grid-cols-3">
          {testimonials.map((t, i) => {
            const [big, ...rest] = t.metric.split(" ");
            const caption = rest.join(" ");
            const isRating = caption.toLowerCase().includes("rating");
            ...
            return (
              <Reveal key={t.author} delay={i * 0.08} className="bg-white">
                <figure className="flex h-full flex-col p-6 sm:p-8">
                  ...
                  <p className="tighter flex items-center gap-1.5 text-[clamp(1.6rem,2.6vw,2rem)] leading-none font-medium text-ink-950">
                    {big}
                    {isRating && (
                      <Star
                        className="size-[0.6em] text-gold-400"
                        fill="currentColor"
                        strokeWidth={0}
                      />
                    )}
                  ...
                  <figcaption className="mt-7 flex items-center gap-3">
                    <span className="relative shrink-0">
                      {/* Avatar ilustrat (mockup) — nu e o fotografie reală. */}
                      <img
                        src={`https://api.dicebear.com/9.x/notionists-neutral/svg?seed=${encodeURIComponent(t.author)}&backgroundColor=f4f1ea`}
                        alt=""
                        width={44}
                        height={44}
                        loading="lazy"
